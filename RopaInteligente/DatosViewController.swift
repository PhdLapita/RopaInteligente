...
class DatosViewController: UIViewController ,BLEDelegate  {
  let bluetoothManager = BTManager.getInstance()
    ...
    @IBAction func clicFYIpulso(_ sender: UIButton) {
    
    }
    
    @IBAction func clicFYItemp(_ sender: UIButton) {
        
    }
    
    @IBAction func clicFYIoximetro(_ sender: UIButton) {
        
    }
    
    @IBAction func clicFYIestado(_ sender: UIButton) {
    ...
    func bleDidReceiveData(_ data: Data?){
       ...
       dato = String(data: data!, encoding: .utf8)!
        if(dato.characters.contains("/") && dato.characters.contains("&")){
            ...
            txtTemp.text = dato[rangotemp]
            txtPulso.text = dato[rangopulso]
            txtOximetro.text = dato[rangooxi]
            if contador == 0{
   ...
   /*    if(string! == "1"){
        imgEstado.image = UIImage(named: "parado")
       }else{
         imgEstado.image = UIImage(named: "descanso")
        }*/
    }
